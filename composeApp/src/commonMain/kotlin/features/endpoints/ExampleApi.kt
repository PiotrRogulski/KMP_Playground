...
class ExampleApi(
    ...
    suspend fun getResources(page: Int = 1, perPage: Int = 4) =
        paginated<Resource>("unknown", page, perPage)

    suspend fun getResource(id: Int) = single<Resource>("unknown/$id")
}

private suspend inline fun <reified T> ExampleApi.single(query: String): SingleResponse<T> =
    client.get(query).body()
